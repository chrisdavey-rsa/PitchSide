/**
 * Shared API-Sports client for Node scripts.
 * Enforces per-sport daily budget via public.reserve_api_quota RPC
 * (football and rugby each get DAILY_BUDGET_CAP independently).
 *
 * Edge Functions use a ported copy at:
 *   supabase/functions/_shared/apiSportsClient.ts
 * Keep the two in sync when changing budget / throttle behaviour.
 */
...
/** Classify API-Sports `errors` payload (often present even on HTTP 200). */
export function classifyApiErrors(
  ...
  // Plan / entitlement messages (include "Free plans do not have access…").
  if (
  ...
  // Everything else with an errors object is a bad request / missing param.
  return "validation_error";
}

/**
 * Aug–May domestic seasons: season year = start year.
 * Free plan currently allows 2022–2024 only — clamp when preferred year is ahead.
 */
...
export class ApiSportsClient {
  ...
  async getFixtureCheck(
    sport: Sport,
    leagueId: number,
    fixtureDate: string,
  ): Promise<{ fixtures_found: number } | null> {
    const { data, error } = await this.opts.supabase
      .from("api_fixture_checks")
      .select("fixtures_found")
      .eq("sport", sport)
      .eq("league_id", leagueId)
      .eq("fixture_date", fixtureDate)
      .eq("checked_on", utcDay())
      .maybeSingle();
    if (error) {
      console.warn(`[api-sports] fixture check read failed: ${error.message}`);
      return null;
    }
    return data;
  }

  async recordFixtureCheck(
    sport: Sport,
    leagueId: number,
    fixtureDate: string,
    fixturesFound: number,
  ): Promise<void> {
    const { error } = await this.opts.supabase.from("api_fixture_checks").upsert(
      {
        sport,
        league_id: leagueId,
    ...
    if (error) {
      console.warn(`[api-sports] fixture check write failed: ${error.message}`);
    }
  }
